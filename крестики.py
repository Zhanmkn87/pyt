...
size_block=100
marqin = 15
width = heigth=size_block*3+marqin*4
...
while True:
    for event in pygame.event.get():
        if event.type==pygame.QUIT:
            ...
            col=x_mouse // (size_block+marqin)
            row=y_mouse // (size_block+marqin)
            if mas[row][col]==0:
                if query%2==0:
                    mas[row][col]='x'
                else:
            ...
            for row in range(3):
                for col in range(3):
                    if mas[row][col]=='x':
                        ...
                        color=white
                    x=col*size_block+(col+1)*marqin
                    y=row*size_block+(row+1)*marqin
